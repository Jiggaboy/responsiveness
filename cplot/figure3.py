...
def main():
    ...
    gs = fig.add_gridspec(nrows=4, ncols=3) #width_ratios=()
    fig.subplots_adjust(
        # left=0.06,
        # right=0.95,
        # bottom=0.1,
    ...
    for stat in ("\N{UPWARDS ARROW}", "\N{DOWNWARDS ARROW}"):
        for tag in hue_order:
            labels.append(rf"mean delay ({stat}{Label[tag]})")
    ...
    for s, stim_rep in enumerate(stim_reps):
        ...
        # df_rates_tmp  = df_rates_tmp.xs((plot_mean), level=("mean"))
        # df_delays_tmp = df_delays_tmp.xs((plot_mean), level=("mean"))
