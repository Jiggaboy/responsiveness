...
def raster_spikes(spike_times:np.ndarray, senders:np.ndarray, num:str=None):
    logger.info("Init figure...")
    f, (ax_spikes, ax_hist) = plt.subplots(nrows=2, height_ratios=[3, 1], sharex=True, tight_layout=True, num=num)
    # ax_spikes.set_title(f"{pre_mean}+-{pre_std} to {post_mean}+-{post_std}")
    ax_spikes.scatter(spike_times, senders, c="blue", marker=".")
    ax_spikes.axvline(warmup+duration, c="k", ls="--")
    ax_spikes.set_ylabel("Neuron ID")

    logger.info("Plot PSTH...")
    t_bins = np.arange(warmup, 2*duration+warmup+hist_binwidth, float(hist_binwidth))
    spikecounts, _ = np.histogram(spike_times, bins=t_bins)

    heights = spikecounts / (hist_binwidth*1e-3 * N)
    plt.bar(t_bins[:-1] + hist_binwidth/2, heights, width=hist_binwidth, color="blue", edgecolor="black")
    ax_hist.axvline(duration+warmup, c="k", ls="--")
    
    t_bins = np.arange(duration, 2*duration, float(hist_binwidth)) + warmup
    spikecounts, _ = np.histogram(spike_times, bins=t_bins)
    
    SEM, delay = get_transient(spikecounts)
    plt.axvline(delay * hist_binwidth + warmup + duration, c="k", lw=3)

    ax_hist.set_ylabel("Rate (spks/s)")
    ax_hist.set_xlabel("Time (ms)")
    ax_hist.set_xlim(duration+warmup-50, duration+warmup+50)

    # Estimate the Firing rate (pre and post)
    buffer = duration * 0.25
    mask = np.logical_and(spike_times >= warmup + buffer, spike_times < warmup + duration)
    pre_FR = FR_from_spikecount(np.count_nonzero(spike_times[mask]), N, duration-buffer)
    mask = np.logical_and(spike_times >= warmup + duration + buffer + hist_binwidth, spike_times < 2*duration + warmup + hist_binwidth)
    post_FR = FR_from_spikecount(np.count_nonzero(spike_times[mask]), N, duration-buffer)
    plt.axhline(pre_FR, c="g")
    plt.axhline(post_FR, c="r")

    # Estimate the FR based on the latter half of the data
    t_bins = np.arange(duration+warmup+hist_binwidth, 2*duration+warmup+hist_binwidth, float(hist_binwidth))
    spikecounts, _ = np.histogram(spike_times, bins=t_bins)
    spikecounts = spikecounts.astype(float) / (hist_binwidth*1e-3 * N)
    half = spikecounts.size // 2
    SS_mean = spikecounts[half:].mean()
    SS_std = spikecounts[half:].std(ddof=1)
    plt.axhline(SS_mean, c="r", lw=2)
    plt.axhline(SS_mean+SS_std, c="r", ls="--")
    plt.axhline(SS_mean-SS_std, c="r", ls="--")
